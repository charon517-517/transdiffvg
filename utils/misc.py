...
def collate_fn(raw_batch):
    ...
    clip_ids = torch.tensor(raw_batch[5])
    clip_mask = torch.tensor(raw_batch[6])
    clip_data  =NestedTensor(clip_ids, clip_mask)
    raw_image = [s for s in list(raw_batch[7])]
    img_names = list(raw_batch[8])
    unet_feat = list(raw_batch[9])
    phrases = list(raw_batch[10])
    batch = [img_data, text_data, bbox, clip_data, raw_image, img_names, unet_feat, phrases]
    return tuple(batch)
