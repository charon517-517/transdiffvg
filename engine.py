...
def train_one_epoch(args, model: torch.nn.Module, data_loader: Iterable, 
    ...
    for batch in metric_logger.log_every(data_loader, print_freq, header):
        img_data, text_data, target, clip_data, raw_image, img_names, unet_feats, phrases = batch
        ...
        raw_image = [r.to(device) for r in raw_image]
        unet_feats = [feat.to(device) for feat in unet_feats]
        forwd_start_t = time.time()
        # model forward
        output = model(img_data, text_data, clip_text=clip_data.decompose()[0], raw_img=raw_image, use_unet=args.use_unet, img_names=img_names, unet_feats=unet_feats, phrases=phrases)
        # print(f"forward cost {time.time()-forwd_start_t}s")
...
def validate(args, model: torch.nn.Module, data_loader: Iterable, device: torch.device):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        img_data, text_data, target, clip_data, raw_img, img_names, unet_feat, phrases = batch
        # print(f"val feature {unet_feat}")
        ...
        target = target.to(device)
        clip_data = clip_data.to(device)
        raw_img = [r.to(device) for r in raw_img]
        pred_boxes = model(img_data, text_data, clip_text = clip_data.decompose()[0], raw_img=raw_img, use_unet=args.use_unet, img_names=img_names, unet_feats=unet_feat, phrases=phrases)
...
def evaluate(args, model: torch.nn.Module, data_loader: Iterable, device: torch.device):
    ...
    for _, batch in enumerate(tqdm(data_loader)):
        ...
        clip_data = clip_data.to(device)
        raw_img = [r.to(device) for r in raw_img]
        output = model(img_data, text_data, clip_text = clip_data.decompose()[0], raw_img=raw_img, unet_feats=unet_feats)
